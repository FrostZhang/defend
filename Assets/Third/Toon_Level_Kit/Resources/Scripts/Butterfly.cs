...
public class Butterfly : MonoBehaviour {
	...
	void Start(){
		...
		for(int i=0;i<butterflyCount;i++){
			GameObject go= (GameObject)Instantiate( butterflyObjects[ Random.Range(0,butterflyObjects.Length-1)],
                new Vector3( transform.position.x + Random.Range( -zoneSize.x,zoneSize.x)/2,
                transform.position.y + Random.Range( -zoneSize.y,zoneSize.y)/2,transform.position.z + 
                Random.Range( -zoneSize.z,zoneSize.z)/2),Quaternion.identity,transform);
